...
def _write_field(out, field):
    ...
    # Time: O(n) - encode (if str) and copy n bytes.
    # Space: O(n) - the encoded copy.
...
def _decode_map(blob):
    ...
    # Time: O(n + b) - read every field once.
    # Space: O(n + b) - the rebuilt map.
    reader = _Reader(blob)
...
def _join_segments(segments):
    ...
    # Time: O(t log t + s) - sort by index header, then concatenate.
    # Space: O(s).
...
#   We need to save a dictionary to "disk" and read it back, even when
#   keys or values contain tricky bytes (spaces, '|', ':', newlines). The
#   trick: never rely on separators. Instead, right before every chunk we
#   write its exact length - like labelling a parcel "5 bytes follow". The
#   reader then grabs exactly that many bytes, so nothing inside can ever be
#   mistaken for a separator (binary-safe).
#     - Part 1: write the whole map as one length-prefixed blob and
#       read it back.
#     - Part 2: chop that blob into fixed-size parcels, each stamped
#       with its index, so they can be stored or shuffled and later
#       sorted back into the right order.
#     - Part 3: keep a saved snapshot plus a running list (the log) of
...
class PersistentKvStore:
    ...
    @staticmethod
    def by(s):
        """Demo helper: Python bytes from a str (ISO-8859-1, 1 char == 1
        byte)."""
        # n = len(s). Time: O(n) - encode n characters.
        # Space: O(n) - the bytes.
    ...
    def py_repr(results):
        ...
        None -> None, tuple -> (...))."""
        # n = total size of results.
        # Time: O(n), Space: O(n) - builds a repr string.
        return repr(results)

    # ---------- Part 1: serialize / deserialize a single snapshot ----------
    @staticmethod
    def simulate_part1(operations):
        """Part 1: serialize/deserialize a whole snapshot to and from a
        label."""
        # m = number of ops, s = current store's serialized size,
        # r = get-results.
        # Time: O(m * s) worst case (serialize/deserialize copy the whole
        # store).
    ...
    def simulate_part2(max_segment_size, operations):
        """Part 2: serialize into size-bounded segments that survive
        reordering."""
        # m = number of ops, s = serialized store size,
        # t = segments per snapshot.
        # Time: O(m * (s + t log t)) worst case.
        # Space: O(s + r) - segments plus the r results.
        store = {}
        ...
        for op in operations:
            ...
            if kind == "put":
                ...
                disk[op[1]] = _split_into_segments(
                    _encode_map(store), max_segment_size
                )
    ...
    def simulate_part3(compact_threshold, operations):
        """Part 3: snapshot + append-only log, replayed on restart and
        folded in on compaction."""
        # m = number of ops, s = store size, L = pending log length,
        # r = results.
        # Time: O(m * s) worst case (compaction/restart re-encode or replay).
        # Space: O(s + L + r) - snapshot, log, and results.
        store = {}
        # Persisted artifacts (survive a restart): a snapshot blob + a
        # pending log.
        ...
        def maybe_compact():
            # Time: O(s) when it compacts (re-encode the whole store),
            # else O(1).
...
if __name__ == "__main__":
    ...
    kv1c = [
        ["put", by("a|b"), by("v:1\n")], ["serialize", "s"],
        ["put", by("a|b"), by("zzz")], ["deserialize", "s"],
        ["get", by("a|b")],
    ]
    print(PersistentKvStore.py_repr(PersistentKvStore.simulate_part1(kv1c)))
    # [b'v:1\n']

    # Test 4: a str value stays str and a bytes value stays bytes.
    kv1d = [
        ["put", "k", "x"], ["put", by("k2"), by("x")],
        ["get", "k"], ["get", by("k2")],
    ]
    print(PersistentKvStore.py_repr(PersistentKvStore.simulate_part1(kv1d)))
    # ['x', b'x']
    ...
    # Test 6: reversing the segment order still reassembles the snapshot
    # correctly.
    kv2b = [
        ["put", by("k1"), by("AAAA")], ["put", by("k2"), by("BBBB")],
        ["serialize", "p"], ["segment_count", "p"],
        ["reorder", "p", [2, 1, 0]],
    ...
    kv2c = [
        ["serialize", "e"], ["segment_count", "e"],
        ["deserialize", "e"], ["get", by("x")],
    ]
    print(PersistentKvStore.py_repr(
        PersistentKvStore.simulate_part2(15, kv2c)))
    # [1, None]
    ...
    # Test 9: hitting the threshold compacts, clearing the log and
    # bumping the count.
    kv3b = [
        ["put", "a", "1"], ["put", "b", "2"],
        ["put", "c", "3"], ["status"],
    ...
    # Test 10: restart after a compaction reloads the snapshot then
    # replays the log.
    kv3c = [
        # compaction here
    ...
    # Test 11: binary-safe snapshot survives compaction + restart
    # (mixed str/bytes).
    kv3d = [
