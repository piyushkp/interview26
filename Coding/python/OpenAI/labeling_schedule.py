...
#   Picture running a workshop where every helper (human) must finish k different
#   tasks, and you want to hand out the work in a tidy, repeatable way.
#     - any_schedule: give each helper k tasks in a row, but start each helper on a
#       different task by rotating the starting point by the helper's number - like
#       dealing cards - so nobody repeats a task and the work spreads out. Model is
#       always m1 because Part 1 only cares about a valid assignment.
#     - prefix_balanced_schedule: instead of finishing one helper at a time, go
#       round by round. In round r every helper does the same task t{r+1} at once,
#       and we rotate which model each helper uses. Because a whole round finishes
#       before the next starts, if you stop reading the list at ANY point everyone
#       has done nearly the same amount - that is what "prefix-balanced" means.
#   No schedule is possible when k distinct tasks cannot fit (k > total_task); when
#   k is 0 there is simply nothing to schedule, so an empty list is the answer.
...
def prefix_balanced_schedule(total_task, total_model, total_human, k):
    ...
    for round_index in range(k):                  # round r -> task t{r+1}
        for human_index in range(total_human):    # human index within the round
            ...
            result.append([f"t{round_index + 1}", f"m{model_index + 1}", f"h{human_index + 1}"])
    return result
...
if __name__ == "__main__":
    ...
    # [['t1', 'm1', 'h1'], ['t2', 'm1', 'h1'], ['t2', 'm1', 'h2'], ['t1', 'm1', 'h2'], ['t1', 'm1', 'h3'], ['t2', 'm1', 'h3']]
    ...
    # [['t1', 'm1', 'h1'], ['t1', 'm2', 'h2']]
    print(prefix_balanced_schedule(2, 3, 2, 2))   # model index rotates each round
    # [['t1', 'm1', 'h1'], ['t1', 'm2', 'h2'], ['t2', 'm2', 'h1'], ['t2', 'm3', 'h2']]
